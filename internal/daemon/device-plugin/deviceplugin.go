...
const (
	...
	DpuResourceName         = DefaultDpuResourceName
...
func newFilteredDeviceHandler(inner dh.DeviceHandler, vfIDs []int32) *filteredDeviceHandler {
	allowed := make(map[int32]struct{}, len(vfIDs))
	for _, id := range vfIDs {
		allowed[id] = struct{}{}
	}
	return &filteredDeviceHandler{inner: inner, allowedVFs: allowed}
}
...
func (h *filteredDeviceHandler) GetDevices() (*dh.DeviceList, error) {
	...
	// VF mode: exclude accelerated: devices, then filter by positional index.
	var vfIDs []string
	for id := range *all {
		if !strings.HasPrefix(id, acceleratedDevicePrefix) {
...
func (m *DevicePluginManager) startPlugin(name string, res devicePluginResource) bool {
	...
	dp := &dpServer{
		...
		deviceHandler: newFilteredDeviceHandler(m.defaultPlugin.deviceHandler, vfIDs),
...
func NewDevicePlugin(vsp plugin.VendorPlugin, dpuMode bool, pm utils.PathManager, opts ...func(*dpServer)) *dpServer {
	devHandler := dpudevicehandler.NewDpuDeviceHandler(vsp, dpudevicehandler.WithDpuMode(dpuMode), dpudevicehandler.WithPathManager(pm))
	dp := &dpServer{
		devices:       make(map[string]pluginapi.Device),
		grpcServer:    grpc.NewServer(),
		log:           ctrl.Log.WithName("DevicePlugin"),
