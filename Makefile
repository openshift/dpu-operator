...
local-buildx: ## Build all container images necessary to run the whole operator
	...
	buildah build $(DPU_OPERATOR_LOCAL_BUILDX_ARGS) --manifest $(DPU_OPERATOR_IMAGE)-manifest --platform linux/amd64,linux/arm64 -v $(GO_CONTAINER_CACHE):/go:z -f Dockerfile.rhel -t $(DPU_OPERATOR_IMAGE)
	buildah manifest rm $(DPU_DAEMON_IMAGE)-manifest || true
	buildah manifest create $(DPU_DAEMON_IMAGE)-manifest
	buildah build $(DPU_OPERATOR_LOCAL_BUILDX_ARGS) --manifest $(DPU_DAEMON_IMAGE)-manifest --platform linux/amd64,linux/arm64 -v $(GO_CONTAINER_CACHE):/go:z -f Dockerfile.daemon.rhel -t $(DPU_DAEMON_IMAGE)
	buildah manifest rm $(MARVELL_VSP_IMAGE)-manifest || true
	buildah manifest create $(MARVELL_VSP_IMAGE)-manifest
	buildah build $(DPU_OPERATOR_LOCAL_BUILDX_ARGS) --manifest $(MARVELL_VSP_IMAGE)-manifest --platform linux/amd64,linux/arm64 -v $(GO_CONTAINER_CACHE):/go:z -f Dockerfile.mrvlVSP.rhel -t $(MARVELL_VSP_IMAGE)
