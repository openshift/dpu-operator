...
func (vsp *mrvlVspServer) getAcceleratedDevices() (map[string]*pb.Device, error) {
	...
	for _, ifName := range rpmNames {
		// Only advertise data-plane RPM ports (>= 25 Gbps). On Marvell cn10k
		// DPUs the host/lab management uplink (when routed through an RPM)
		// runs at 1G or 10G, while the front-panel data-plane ports are
		// 25/50/100/200G. Filtering by speed prevents the device plugin from
		// ever picking the management interface and moving it into a pod
		// netns, which would disconnect the node.
		ok, err := mrvlutils.IsAcceleratedRPMCandidate(ifName)
		if err != nil {
			klog.Warningf("Skipping RPM %s from accelerated pool: cannot determine link speed, treating as host-owned: %v", ifName, err)
			continue
		}
		if !ok {
			speed, _ := mrvlutils.GetLinkSpeedMbps(ifName)
			klog.Warningf("Skipping RPM %s from accelerated pool: link speed %d Mbps is below %d Mbps (likely host management interface)", ifName, speed, mrvlutils.MinAcceleratedRPMSpeedMbps)
			continue
		}
