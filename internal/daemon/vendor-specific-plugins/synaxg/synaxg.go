...
func GetUploadFile() (string, error) {
	commd := "ls /*.tar.gz | tr -d '\n'"
	out, err := execCommand(commd)
	if err != nil {
		return "", err
	}
	file := string(out)
...
func (vsp *sgVspServer) ManualUpgradeSdkFunction(ctx context.Context, in *pb.ManualOperationRequest) (*pb.ManualOperationResponse, error){
	var (
		...
		wg_update.Add(1)
		addr := fmt.Sprintf("%s:%d", cardInfo.CardIp, vduPort)
		go WorkerUpgrade(addr, filePath, &cardReqElem.StatusCode, &wg_update)
