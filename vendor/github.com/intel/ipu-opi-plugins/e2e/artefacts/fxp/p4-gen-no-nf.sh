...
rh_mvp_control.ingress_loopback_table \"vsi=0x$vsi,target_vsi=0x$target_vsi,action=rh_mvp_control.fwd_to_port($vf2_port)\""

            echo "\$P4CP_INSTALL/bin/p4rt-ctl add-entry br0 \
rh_mvp_control.vport_egress_dmac_vsi_table \"vsi=0x$vsi,dmac=0x$dmac,action=rh_mvp_control.fwd_to_port($vf2_port)\""
        fi
    done
done