...
type dpuNetworkNADConfig struct {
...
func (r *DpuNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !net.DeletionTimestamp.IsZero() {
		if controllerutil.ContainsFinalizer(net, dpuNetworkFinalizer) {
			...
			if err := r.deleteAccelNADIfUnused(ctx, net.Name); err != nil {
				logger.Error(err, "Failed to delete accelerated NAD on deletion")
				return ctrl.Result{}, err
			}
	...
	// Ensure a single global accelerated NAD if this network uses acceleration.
	if net.Spec.IsAccelerated {
		if err := r.ensureAccelNAD(ctx); err != nil {
			meta.SetStatusCondition(&net.Status.Conditions, metav1.Condition{
				Type:    "Ready",
				Status:  metav1.ConditionFalse,
				Reason:  "AccelNADError",
				Message: err.Error(),
			})
			_ = r.Status().Update(ctx, net)
			return ctrl.Result{}, err
...
func (r *DpuNetworkReconciler) ensureNAD(ctx context.Context, net *configv1.DpuNetwork, resourceName string) error {
	...
	configBytes, err := json.Marshal(nadConfig)
	if err != nil {
		return fmt.Errorf("failed to marshal NAD config: %w", err)
	}
...
func (r *DpuNetworkReconciler) ensureAccelNAD(ctx context.Context) error {
	key := types.NamespacedName{Name: dpuAccelNADName, Namespace: defaultDpuNetworkNADNamespace}
	...
	nad.Annotations["k8s.v1.cni.cncf.io/resourceName"] = "openshift.io/dpu-accelerated"
	...
	configBytes, err := json.Marshal(nadConfig)
	if err != nil {
		return fmt.Errorf("failed to marshal accelerated NAD config: %w", err)
	}
	nad.Spec.Config = string(configBytes)
...
func (r *DpuNetworkReconciler) deleteAccelNADIfUnused(ctx context.Context, deletingNetwork string) error {
	list := &configv1.DpuNetworkList{}
	if err := r.List(ctx, list); err != nil {
		return err
	}
	for _, n := range list.Items {
		if n.Name == deletingNetwork {
			continue
		}
		if n.Spec.IsAccelerated {
			return nil
		}
	}
