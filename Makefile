...
test: podman-check manifests generate fmt vet envtest ginkgo
	FAST_TEST=false KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" timeout 30m $(GINKGO) --repeat 4 $(if $(TEST_FOCUS),-focus $(TEST_FOCUS),) ./internal/... ./pkgs/... ./api/v1/... -coverprofile cover.out
...
.PHONY: build
build: manifests generate fmt vet build-manager build-daemon build-intel-vsp build-marvell-vsp build-webhook
...
WEBHOOK_IMAGE := $(REGISTRY):5000/webhook:dev
...
local-build: ## Build all container images necessary to run the whole operator
	...
	$(CONTAINER_TOOL) build -v $(GO_CONTAINER_CACHE):/go:z -f Dockerfile.webhook.rhel -t $(WEBHOOK_IMAGE)
...
.PHONY: local-buildx
local-buildx: prepare-multi-arch go-cache local-buildx-manager local-buildx-daemon local-buildx-marvell-vsp local-buildx-intel-vsp local-buildx-webhook
...
.PHONY: local-buildx-webhook
local-buildx-webhook: prepare-multi-arch go-cache
	$(call build_image,WEBHOOK_IMAGE,Dockerfile.webhook.rhel)
...
.PHONY: local-buildx-incremental-webhook
local-buildx-incremental-webhook: prepare-multi-arch go-cache
	GOARCH=arm64 $(MAKE) build-webhook
	GOARCH=amd64 $(MAKE) build-webhook
	$(call build_image_incremental,WEBHOOK_IMAGE,Dockerfile.webhook.rhel)

.PHONY: incremental-local-buildx
incremental-local-buildx: prepare-multi-arch go-cache incremental-prep-local-deploy local-buildx-incremental-manager local-buildx-incremental-daemon local-buildx-incremental-marvell-vsp local-buildx-incremental-intel-vsp local-buildx-incremental-webhook
...
local-pushx-incremental: ## Push all container images necessary to run the whole operator
	...
	buildah manifest push --all $(WEBHOOK_IMAGE)-manifest docker://$(INTEL_VSP_IMAGE)
...
local-pushx: ## Push all container images necessary to run the whole operator
	...
	buildah manifest push --all $(MARVELL_VSP_IMAGE)-manifest docker://$(MARVELL_VSP_IMAGE)
	buildah manifest push --all $(INTEL_VSP_IMAGE)-manifest docker://$(INTEL_VSP_IMAGE)
	buildah manifest push --all $(WEBHOOK_IMAGE)-manifest docker://$(WEBHOOK_IMAGE)
	...
	buildah manifest push --all $(WEBHOOK_IMAGE)-manifest docker://$(WEBHOOK_IMAGE)-base
...
local-push: ## Push all container images necessary to run the whole operator
	...
	$(CONTAINER_TOOL) push $(WEBHOOK_IMAGE)
