...
func cmdAdd(args *skel.CmdArgs) error {
	...
	c := pb.NewBridgePortServiceClient(conn)
	...
	createRequest := &pb.CreateBridgePortRequest{
		BridgePort: &pb.BridgePort{
			...
			Spec: &pb.BridgePortSpec{
				Ptype:          1,
				MacAddress:     []byte(mac),
				LogicalBridges: []string{fmt.Sprint(netConf.VFID + 2)},
			},
		},
	}
	...
	if err != nil {
		return fmt.Errorf("could not create bridge port: %v", err)
	}
	log.Printf("BridgePort Name: %s", r.GetName())
...
func cmdDel(args *skel.CmdArgs) error {
	...
	*/
	...
	if err != nil {
		return fmt.Errorf("could not delete bridge port: %v", err)
	}
	log.Printf("BridgePort Deleted")
