...
var _ = Describe("DpuNetwork Controller", Ordered, func() {
	...
	AfterEach(func() {
		...
		err := mgr.GetClient().Get(context.Background(), types.NamespacedName{Name: "net1"}, net)
		if err == nil {
			Expect(mgr.GetClient().Delete(context.Background(), net)).To(Succeed())
			Eventually(func() error {
				return mgr.GetClient().Get(context.Background(), types.NamespacedName{Name: "net1"}, net)
			}, testutils.TestAPITimeout*5, testutils.TestRetryInterval).ShouldNot(Succeed())
		}
	})
